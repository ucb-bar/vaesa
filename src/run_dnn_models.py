...
import util
from cosa.cosa import run_timeloop
from cosa.cosa_input_objs import Prob
...
def construct_argparser():
    ...
    parser.add_argument('--buf_ratios_file',
                        type=str,
                        help='Pickle file with buffer partitioning ratios')
...
def gen_prob(workload_dir, model, dnn_def_path):
    ...
    layer_defs = util.parse_json(dnn_def_path)
    ...
    for layer_idx, layer_def in enumerate(layer_defs):
        ...
        util.store_yaml(prob_path, prob)
        files.append(file_name)
    layer_def_path = model_dir / 'unique_layers.yaml'
    util.store_yaml(layer_def_path, files)
    print(layer_def_path)
    

def run_dnn_models(workload_dir, arch_path, mapspace_path, output_dir, model, buf_ratios, layer_idx, dnn_def_path=None):
    if buf_ratios == None:
        buf_ratios = [[0, 0, 1], [1, 0, 0], [0, 1, 0], [0, 0.25, 0.75]]
    buf_str = "_" + "_".join([str(r) for r in np.array(buf_ratios).flatten()])
    cycle_result_path = pathlib.Path(output_dir) / f'results_{arch_path.stem}{buf_str}_cycle.json'
    ...
    for model_str in model_strs:
        ...
        for idx, layer in enumerate(layers):
            ...
            print(prob_path)
            status_dict = run_timeloop(prob_path, arch_path, mapspace_path, output_dir, buf_ratios)
    ...
    for k in full_results:
        full_result_path = pathlib.Path(output_dir) / f'results_{arch_path.stem}{buf_str}_{k}.json'
        util.store_json(full_result_path, full_results[k], indent=2)
...
if __name__ == "__main__":
    ...
    buf_ratios = None
    if args.buf_ratios_file:
        with open(args.buf_ratios_file, "rb") as f:
            buf_ratios = pickle.load(f)

    run_dnn_models(workload_dir, arch_path, mapspace_path, output_dir, model, buf_ratios, layer_idx, dnn_def_path=args.dnn_def_path)
